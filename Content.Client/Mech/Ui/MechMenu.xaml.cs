...
public sealed partial class MechMenu : FancyWindow
{
    ...
    public void UpdateEquipmentView(MechBoundUiState state) // _FtC Mech UI Fix
    {
        if (!_ent.TryGetComponent<MechComponent>(_mech, out var mechComp))
            return;

        EquipmentControlContainer.RemoveAllChildren();  // _FtC Mech UI Fix start (строка наша)
        var list = state.EquipmentStates.Keys;
        foreach (var item in list)
        {
            var ent = _ent.GetEntity(item);             // _FtC Mech UI Fix end (строка наша)
